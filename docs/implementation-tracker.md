...
- 2026-09-01: Windows validation capability je `implemented` v `packages/core/src/windows-worker.ts`, `apps/studio-api/src/routes/windows-runner-routes.ts`, `packages/db/src/windows-worker-repository.ts` a `apps/windows-runner/src`; transport, lease, auth, audit, cancel, result flow, fixture executor a pinned Unreal adapter jsou `tested` v `apps/studio-api/src/routes/windows-runner-routes.integration.test.ts`, `apps/windows-runner/src/*.test.ts` a `packages/db/src/windows-worker-repository.integration.test.ts`. Produkcni overeni zatim neexistuje. Realna BOREK-FILIP validace zustava `deferred` do lokalni manualni session se samostatnym approval dlouhe prace a pinned/probed Unreal tooling; fixture evidence ji nenahrazuje. Manualni finalni audit je take `deferred` a human-triggered.
