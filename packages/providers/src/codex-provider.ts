...
function validationChecksJsonSchema(): Record<string, unknown> {
  return {
    ...
    items: {
      type: 'object',
      additionalProperties: false,
      required: ['kind', 'command', 'shell', 'target', 'requiredCapabilities', 'continueOnFailure', 'category', 'criterion', 'rationale', 'timeoutMinutes', 'windowsAdapter', 'realEngineEvidence'],
      properties: {
        ...
        windowsAdapter: {
          anyOf: [
            ...
            ,{ type: 'object', additionalProperties: false,
              required: ['kind', 'runtimeKind', 'profileId', 'executablePath', 'workingDirectoryRelativePath', 'args', 'artifactRelativePath', 'settleSeconds'],
              properties: { kind: { type: 'string', const: 'runtime-capture' }, runtimeKind: { type: 'string', enum: ['editor', 'built-application'] }, profileId: { type: 'string' }, executablePath: { type: 'string' }, workingDirectoryRelativePath: { type: 'string' }, args: { type: 'array', items: { type: 'string' } }, artifactRelativePath: { type: 'string' }, settleSeconds: { type: 'integer', minimum: 1, maximum: 300 } } }
          ]
        },
        realEngineEvidence: { anyOf: [{ type: 'null' }, { type: 'object', additionalProperties: false,
          required: ['classification', 'buildId', 'scenario', 'settings', 'shippingExecutable'], properties: {
            classification: { type: 'string', enum: ['automated-scenario', 'benchmark', 'soak', 'build-validation', 'capture', 'shipping'] },
            buildId: { type: 'string' }, scenario: { type: 'string' }, settings: { type: 'object', additionalProperties: false, properties: {}, required: [] },
            shippingExecutable: { anyOf: [{ type: 'null' }, { type: 'object', additionalProperties: false, required: ['relativePath', 'sha256', 'platform', 'configuration', 'current'],
              properties: { relativePath: { type: 'string' }, sha256: { type: 'string' }, platform: { type: 'string', const: 'Win64' }, configuration: { type: 'string', const: 'Shipping' }, current: { type: 'boolean' } } }] }
          } }] }
      }
    }
  };
    ...
    const messages: Array<{ role: 'system' | 'user' | 'assistant'; content: string }> = [
      {
        ...
        content:
          ...
          'After editing, propose authoritative validationChecks. An empty array explicitly means no executable validation is applicable. For every command select shell, target (local or windows), requiredCapabilities, continueOnFailure, and timeoutMinutes. Use target windows only when the check genuinely requires Windows or Windows-only tooling. For Windows checks provide windowsAdapter only for a complete fixture, Unreal, or runtime-capture profile; otherwise use null for manual-local deferral. A runtime capture must require interactive-desktop and gpu, identify its PNG artifact, and use capture-classified realEngineEvidence. Inspect a returned capture by artifact hash, repair concrete visible defects, and request another capture only after the result tree or capture settings changed. ForgeMind executes local commands exactly as returned; Windows checks execute only through the typed adapter. ' +
          'For a Windows check that produces real-engine scenario, benchmark, soak, build-validation, capture, or Shipping evidence, include realEngineEvidence with its distinct classification, buildId, scenario, and settings; Shipping must also identify the current Win64 Shipping executable and hash. ' +
      ...
      },
      {
        ...
        content: [
          ...
          input.visualEvidence?.length ? `Runtime captures available for visual inspection (open these actual image files before repairing):\n${input.visualEvidence.map((item) => `${item.localPath} sha256=${item.artifactHash} tree=${item.resultTreeSha} build=${item.buildId} scene=${item.scene} settings=${JSON.stringify(item.settings)}`).join('\n')}` : '',
        ]
          .filter(Boolean)
          .join('\n')
      }
...
export function buildCodexImplementationPrompt(input: ImplementInput, continueSession = false): string {
  return [
    ...
    'Set outcome to changes_made when repository changes are required, already_satisfied when the repository already meets the task, or blocked only for a concrete external blocker that prevents further progress.',
    'After editing, return authoritative validationChecks. An empty array explicitly means no executable validation is applicable. For every command select shell (system, powershell, cmd, bash, or sh), target (local or windows), requiredCapabilities, continueOnFailure, and a realistic timeoutMinutes from 1 to 600. Use target windows only when the check genuinely requires Windows or Windows-only tooling. For each Windows check set windowsAdapter to a complete fixture-validation, unreal-validation, or runtime-capture profile only when applicable; otherwise set it to null for manual-local deferral. Runtime capture requires interactive-desktop and gpu capabilities, a PNG artifact path, and capture-classified realEngineEvidence. Inspect captures by artifact hash, repair concrete visible defects, and request a new capture only after the result tree or capture settings changed. ForgeMind executes local commands exactly as returned; Windows checks execute only through the typed adapter. Do not use environment-only smoke checks such as node --version unless the task explicitly requires them.',
    'For Windows real-engine scenario, benchmark, soak, build-validation, capture, or Shipping results, attach realEngineEvidence to the validation check with its exact classification, buildId, scenario, and settings. Shipping also requires the current Win64 Shipping executable path and SHA-256.',
    ...
    input.visualEvidence?.length ? `Runtime captures available for visual inspection. Open each image file before repairing:\n${input.visualEvidence.map((item) => `${item.localPath} sha256=${item.artifactHash} tree=${item.resultTreeSha} build=${item.buildId} scene=${item.scene} settings=${JSON.stringify(item.settings)}`).join('\n')}` : '',
  ]
    .filter(Boolean)
    .join('\n\n');
}
