...
Pred vytvorenim relace runner bez AI turnu overi aktivni Codex OAuth prihlaseni, nacte modely skutecne dostupne lokalnimu uctu a vybere jeho vychozi model. Explicitni `CODEX_MODEL` musi byt v tomto seznamu. `FORGEMIND_UNREAL_EXECUTABLE` se overuje pomoci `Engine/Build/Build.version` a skutecneho headless Python commandletu nad docasnym projektem se stejnym `EngineAssociation`; MSVC probe pres `vswhere` a `vcvars64.bat` skutecne zkompiluje x64 objekt a npm se na Windows spousti pres `cmd.exe`. Kazdy probe ma viditelny prubeh a ohranicenou zivotnost procesu. Neuspech povinne capability se nejdrive publikuje jako diagnostika a potom ukonci prikaz nenulovym kodem, takze se nespusti relace ani neclaimne task. `probe --json` navic vypise uplnou evidence; bez prepinace zustava vystup kratky. UnrealEditor lze behem authoringu spustit jen strukturovanym nastrojem nad timto probed executable; obecny process nastroj jej odmitne. Operator aktivuje obnovovanou foreground relaci prikazem `session start --project <uuid>`; `session drain` zastavi nove claimy a necha aktualni ulohu dobehnout, zatimco `session stop` relaci zrusi.
